...
local screenLock = 'slock xset dpms force off'
...
-- {{{ Key bindings
globalkeys = my_table.join(awful.key({}, "Print", function()
        awful.util.spawn("scrot -e 'mv $f ~/Pictures/Screenshot 2>/dev/null'")
    end, {
        description = "take a screenshot",
        group = "hotkeys"
    }), -- X screen sleep
    -- awful.key({modkey}, "F1", function()
    --     os.execute('systemctl hybrid-sleep')
    -- end, {
    --     description = "sleep",
    --     group = "hotkeys"
    -- }),
    ...
    awful.key({ modkey }, "Return", function()
        SpawnOrFocusClient("Alacritty", terminal, false)
    end, {
    ...
    if beautiful.cal then
        beautiful.cal.show(5)
    end
end, {
    description = "show calendar",
    group = "widgets"
}), ]]
    --[[     awful.key({ altkey, }, "h", function () if beautiful.fs then beautiful.fs.show(5) end end,
    ...
    awful.key({}, "#233", function()
        -- 需要用户需要加入到video组, 且安装light
        os.execute('light -A 10')
    end, {
        description = "blacklight up",
        group = "hotkeys"
    }), awful.key({}, "#232", function()
        os.execute("light -U 10")
    end, {
        description = "blacklight down",
        group = "hotkeys"
    }), -- ALSA volume control12
    awful.key({}, "#123", function()
        ...
        os.execute(string.format("amixer set Master toggle; amixer set Speaker  unmute;",
...
clientkeys = my_table.join( --[[     awful.key({ altkey, "Conntrol", "Shift" }, "z", lain.util.magnify_client,
        { description = "magnify client", group = "lclient" }), ]]
...
-- 立即关闭屏幕命令: sleep 1; xset dpms force off
-- 自动锁屏待机, 0000 左上、右上、左下、右下, 鼠标放到左上会忽略
run_once({ "xautolock -time 10 -corners -000 -locker '" .. screenLock .. "' &" })
-- 自动关闭屏幕
os.execute("xset dpms 600 600 600")
-- 键盘频率
...
-- 翻墙
os.execute("pgrep v2raya || v2raya --lite -a 127.0.0.1:1081 &")
