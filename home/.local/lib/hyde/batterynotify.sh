...
fn_status() {
    ...
    "Discharging")
        ...
        if (( battery_percentage <= battery_low_threshold )); then
            prev_status=$battery_status
            steps=$(printf "%1d" $(((battery_percentage + 5) / 10 * 10)))
            notify-send -a "Power" -t 5000 -r 5 -u "CRITICAL" -i "battery-level-${steps:-10}-symbolic" "Charger Plug Out" "Battery is at $battery_percentage%."
            $execute_discharging
        fi
        fn_percentage
        ;;
...
fn_status_change() { # Handle when status changes
    ...
    if [ "$battery_status" != "$last_battery_status" ] || [ "$battery_percentage" != "$last_battery_percentage" ]; then
        ...
        echo "last_battery_status: " $last_battery_status
        echo "battery_percentage: " $battery_percentage
        echo "last_battery_percentage: " $last_battery_percentage
